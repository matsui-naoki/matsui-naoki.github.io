  ...
  const renderMarkdown = (content: string) => {
    ...
    html = html.replace(tableRegex, (match, header, body) => {
      ...
      const headerCells = headers.map((h: string) => 
        `<th class="border border-gray-700 px-4 py-2 bg-gray-800 text-green-400 font-semibold">${h.trim()}</th>`
      ).join('')
    ...
    // Headers
    html = html.replace(/^#### (.*$)/gim, '<h4 class="text-lg font-semibold mt-4 mb-2 text-green-400">$1</h4>')
    html = html.replace(/^### (.*$)/gim, '<h3 class="text-xl font-bold mt-6 mb-3 text-green-300">$1</h3>')
    html = html.replace(/^## (.*$)/gim, '<h2 class="text-2xl font-bold mt-8 mb-4 text-white">$1</h2>')
    html = html.replace(/^# (.*$)/gim, '<h1 class="text-3xl font-black mt-8 mb-6 bg-gradient-to-r from-green-400 to-teal-400 bg-clip-text text-transparent">$1</h1>')
    
    // Bold and italic
    html = html.replace(/\*\*\*(.*?)\*\*\*/g, '<strong class="font-bold text-green-300"><em>$1</em></strong>')
    html = html.replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold text-green-300">$1</strong>')
    html = html.replace(/\*(.*?)\*/g, '<em class="italic">$1</em>')
    
    // Lists - IMPORTANT: Make text visible with text-gray-300
    ...
    // Code blocks
    html = html.replace(/```([^`]+)```/g, '<pre class="bg-gray-900 border border-gray-700 rounded-lg p-4 my-4 overflow-x-auto"><code class="text-green-300 font-mono text-sm">$1</code></pre>')
    html = html.replace(/`([^`]+)`/g, '<code class="bg-gray-800 text-green-300 px-1 py-0.5 rounded font-mono text-sm">$1</code>')
    
    // Horizontal rules
    html = html.replace(/^---$/gim, '<hr class="border-gray-700 my-8">')
    
    // Blockquotes
    html = html.replace(/^> (.*$)/gim, '<blockquote class="border-l-4 border-green-400 pl-4 my-4 text-gray-400 italic">$1</blockquote>')
    ...
    // Links
    html = html.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-green-400 hover:text-green-300 underline">$1</a>')
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <aside className="hidden lg:block w-80 bg-gray-900/50 border-r border-gray-800">
          <div className="sticky top-20 p-6">
            ...
            <nav className="space-y-2">
              {markdownFiles.map((file, index) => (
                ...
                >
                  ...
                  {selectedContent.id === file.id && (
                    <div className="mt-2 h-1 bg-gradient-to-r from-green-400 to-teal-400 rounded-full"></div>
                  )}
                </motion.button>
              ))}
            </nav>
            ...
            <div className="mt-8 p-4 bg-gray-800/30 rounded-lg border border-gray-700/30">
              <h3 className="text-sm font-bold text-green-400 mb-2">学習のポイント</h3>
      ...
      <div className="fixed bottom-8 right-8 space-y-4">
        <button
          ...
          className="bg-gray-800/90 backdrop-blur-sm text-green-400 p-3 rounded-full shadow-lg hover:bg-gray-700/90 transition-colors border border-green-400/30"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
