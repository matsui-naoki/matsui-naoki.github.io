        ...
        <div className="flex items-center space-x-2">
          ...
          {isLong && (
            <button
              ...
              className="text-xs px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded transition-colors duration-200"
            >
...
export default function ResearchSkillsPage() {
  ...
  const renderMarkdown = (content: string) => {
    ...
    html = html.replace(/^### (.*$)/gim, '<h3 class="text-xl font-bold mt-6 mb-3 text-blue-300">$1</h3>')
    html = html.replace(/^## (.*$)/gim, '<h2 class="text-2xl font-bold mt-8 mb-4 text-white">$1</h2>')
    html = html.replace(/^# (.*$)/gim, '<h1 class="text-3xl font-black mt-8 mb-6 bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">$1</h1>')
    ...
    // Inline code
    html = html.replace(/`([^`]+)`/g, '<code class="bg-gray-800 text-blue-300 px-1 py-0.5 rounded font-mono text-sm">$1</code>')
            ...
            <nav className="space-y-2">
              {markdownFiles.map((file, index) => (
                ...
                >
                  ...
                  {selectedContent.id === file.id && (
                    <div className="mt-2 h-1 bg-gradient-to-r from-blue-400 to-indigo-400 rounded-full"></div>
                  )}
                </motion.button>
              ))}
            </nav>
